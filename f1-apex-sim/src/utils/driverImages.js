export const STATIC_TEAM_COLORS = {
  mercedes: '#00D2BE',
  ferrari: '#DC0000',
  mclaren: '#FF8700',
  red_bull: '#0600EF',
...
export const getDriverHeadshot = (familyName = '', teamName = '', year = '2026') => {
  if (!familyName) return null;
  const nameLower = familyName.toLowerCase();
  
  if (Number(year) === 2026 && teamName) {
    const code = DRIVER_CODES[nameLower];
    if (code) {
  ...
  return DRIVER_HEADSHOTS[nameLower] ?? null;
};

export const getDriverAbbrev = (driver) => {
  if (!driver) return '???';
  if (driver.code) return driver.code.toUpperCase();
  const surname = driver.familyName ?? driver.lastName ?? driver.name ?? '';
  return surname.slice(0, 3).toUpperCase() || 'DRV';
};
